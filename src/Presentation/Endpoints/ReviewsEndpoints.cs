...
public static class ReviewsEndpoints
{
    public static WebApplication MapReviewEndpoints(this WebApplication app)
    {
        ...
        _ = root.MapGet("stars/{stars}", GetReviewByStar)
           .Produces<Entities.ReviewByStar>()
           .ProducesValidationProblem()
           .ProducesProblem(StatusCodes.Status404NotFound)
           .ProducesProblem(StatusCodes.Status500InternalServerError)
           .ProducesValidationProblem()
           .WithSummary("Lookup a Review by its Stars")
           .WithDescription("\n    GET /review by stars/1 - 5");
    ...
    public static async Task<IResult> GetReviewByStar([Validate][FromRoute] int stars, [FromServices] IMediator mediator)
    {
        try
        {
            return Results.Ok(await mediator.Send(new Queries.GetReviewByStar.GetReviewByStarQuery
            {
                Stars = stars
            }));
        }
